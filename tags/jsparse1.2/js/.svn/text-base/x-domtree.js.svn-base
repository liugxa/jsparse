...
function DefaultDomTree(_reader){
	...
	this.init = function(_rootName){
		...
		dt_r.setLayer(layer);
		layer.addElement(dt_r);

		//save element & layer
		dt_r.id = elements.size();
		elements.put(dt_r.id , dt_r);
		layers.put(0 , layer);
	...
	this.addElement = function(_fid , _eName){
		...
		var layer = layers.get(f.layer.id + 1);
		e.layer = layer;
		e.id = elements.size();
	...
	_addChildrens = function(_parse , _element , _dt_e , _layerId){
		...
		for(var i=0 ; i < childrens.length ; i++){
			...
			dt_c.setLayer(layer);
			layer.addElement(dt_c);

			//save element
			dt_c.id = elements.size();
			elements.put(dt_c.id , dt_c);
			
			layers.put(_layerId , layer);
	...
	_removeChildrens = function(_dt_e){
		var childrens = _dt_e.getChildrens();
		while(childrens.length > 0 ){
			_removeChildrens(childrens[0]);
		}

		_removeElmentIterm(_dt_e);
	}
	
	_removeElmentIterm = function(_e){

		//remove element
		var f = _e.father;
		f.removeChildren(_e);

		_e.layer.removeElement(_e);
		elements.remove(_e.id);
	}

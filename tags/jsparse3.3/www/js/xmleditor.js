...
function ServiceDefinitionS2H(_domTree , _handle , _imagePrefix){
	...
	_elementValueToTr = function(_element){
		...
		var _input = createHtmlElement("input");
		_input.id = prefix + "-i";
		_input.size = 40;
		_input.className = "input-text";
		_input.value = _element.value;
		//_input.type = hidden;
		...
		_input.onblur = function(){
			...
			if(_element.required != "optional" || _value != ""){
			...
			if(result){
				...
				span.innerHTML = "";
				span.appendChild(createHtmlTextElement(displayValue));
			}else{
				nextStep = false;
				alert(crule.message);
		...
		_input.onchange = function(){
			...
			if(_element.required != "required"){
				if(this.value != ""){
					...
					tr.className = _HTML_CLASS_ATTRIBUTE_DEFAULT + _element.layer;
				}
			}
		}
	...
	_attributeToTr = function(_attribute){
		...
		//add event
		//The "Service Name" property just only read-only when user editr this xml file
		//So, it will use a global-parameter "JS_ACTION" to indetify this operation.
		if(JS_ACTION != "edit" || _attribute.name != "ServiceName"){  
			td[2].onclick = function(){
				if(nextStep){
					var spans = this.getElementsByTagName("span");
					var inputs = this.getElementsByTagName("input");

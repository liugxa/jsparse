...
function ISchema2HtmlForm(_domTree , _handle){
	...
	this.addElementBefore = function(){}
	this.addElementAfter = function(){}
...
function DefaultSchema2HtmlForm(_domTree , _handle){
	...
	this.addElementAtFirst = function(_fid , _eName){
		var f = domTree.getElement(_fid);
		if(domTree.enableAdd(_fid , _eName)){
			var e = domTree.addElement(_fid , _eName);

			var input = document.getElementById(_HTML_ELEMENT_INPUT + "-" + _fid + "-i");
			var flayer = eval(input.value);
			
			var th = document.getElementById(_HTML_ELEMENT_TR + "-" + (_fid + 1));
			if(th != null){
				_printElementItemBefore(th , e , flayer + 1);
			}else{
				_printElementItem(e . flayer + 1);
			}
		}else{
	...
	this.addElementAtLast = function(_fid , _eName){
		var f = domTree.getElement(_fid);
		if(domTree.enableAdd(_fid , _eName)){
			var e = domTree.addElement(_fid , _eName);
			...
			var input = document.getElementById(_HTML_ELEMENT_INPUT + "-" + _fid + "-i");
			var flayer = eval(input.value);
			//alert("input[" + _HTML_ELEMENT_INPUT + "-" + _fid + "-i].value=" + input.value);
			
			//if the element has childrens , u should use insertbefore
			_printElementItem(e , flayer + 1);
		}else{
			throw "validate failure?! can not add these element[" + _eName + "]!!";
		}
	...
	_printElement = function(_element , _layer){
		var layer = _layer;
		_printElementItem(_element , layer);

		//print it's childrens
		var childrens = _element.childrens;
		for(var i=0;i<childrens.length;i++){
			_printElement(childrens[i] , layer + 1);
		}
	}

	_printElementItem = function(_element , _layer){

		//print element 
		var etr = _elementToTr(_element , _layer);
		handle.appendChild(etr);		
		...
		for(var j=0 ; j < atts.length ; j++){
			...
			handle.appendChild(_attributeToTr(attr , _layer));
		}
	}

	//print element before
	_printElementItemBefore = function(_fh , _th , _element , _layer){
		
		//print element
		var etr = _elementToTr(_element , _layer);
		var h = _fh.insertBefore(etr , _th);
		
		//print element's attribute
		var atts = _element.attributes;
		for(var j=0 ; j < atts.length ; j++){
			var attr = atts[j];
			_fh.insertBefore(_attributeToTr(attr , _layer) , _th);
		}
	}

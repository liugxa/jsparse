...
var RULE_VALIDATE_NOT_EMPTY = "This column's value must be NOT empty!";
var RULE_VALIDATE_NOT_INTEGER = "This column's value must be a integer date!";
var RULE_VALIDATE_NOT_DURATION = "This column's valu mus be a duration date!";
		...
		if(_oper == "add"){
			if(this.max != _SCHEMA_ELEMENT_MAXOCCURS_VALUE){
				if((_value + 1) > this.max){
					r = false;
					this.message = "The number of this elements can not exceed " + this.max;
				}
			}
		}
...
function StringRule(_dvalue , _use){
	...
	this.validate = function(_value){
		...
		if(!isRequired(this.use , _value)){
			...
			if(!r) this.message = RULE_VALIDATE_NOT_EMPTY;
		}
		return r;
	}
		...
		for(var i=0 ; i < childrens.length ; i++){
			...
			if(child.nodeType != 8){
				if(child.nodeType != 3){
					...
					dt_c.setContentRule(parse.findElementChildrenContentRule(_element.nodeName , child.nodeName));

...
function DomTreeAttribute(_id , _name , _value){
	...
	//this.status = 1; //-1 -- hidden; 0 -- read-only; 1 -- enable edit

	this.source = "";	
	this.required = "";	//this "function"(add or remove attribute) is not supported
...
var RULE_VALIDATE_NOT_INTEGER = "The value must be an integer type!";
var RULE_VALIDATE_NOT_DURATION = "The value must be a duration type! eg:P12Y03M12D";
...
var RULE_VALIDATE_NON_NEGATIVEINTEGER = "The value must be a nonNegative integer type!";
var RULE_VALIDATE_NOT_UNSINGEDLONG = "The value must be a unsigned long type!";
...
function RestrictPositiveIntegerRule(_dvalue , _use){
...
function RestrictNonNegativeIntegerRule(_dvalue , _use){
...
function DefaultDomTree(_reader){
	...
	_toXml = function(_element){
		var str = "";
		if(_element.isLeaf){
			if(_element.required == "optional" && _element.value == ""){
				//do nothing!
			}else{
	...
	_createNewElement = function(_father, _eName){
		...
		for(var i=0;i<attrs.length;i++){
			...
			if(a.rule.defaultValue != ""){
				a.value = a.rule.defaultValue;
			}else{
				a.value = "NULL";
			}

			//if(a.rule.use == "optional"){ a.value = a.rule.defaultValue;

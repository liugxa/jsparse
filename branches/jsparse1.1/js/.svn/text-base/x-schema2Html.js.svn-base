...
function DefaultSchema2HtmlForm(_domTree , _handle){
	...
	this.addElementAtLast = function(_id , _eName){
		...
		if(domTree.enableAdd(_id , _eName)){
			...
			//var length = e.getChildrens().length;
			//_insertBeforeElement(_id + length + 1 , e , flayer + 1);
		}else{
			throw "validate failure?! can not add these element[" + _eName + "]!!";
		}
	}
	...
	_insertBeforeElement = function(_id , _element , _layer){
		
		var h = document.getElementById(_HTML_ELEMENT_TR + "-" + _id);

		//print element
		var etr = _elementToTr(_element , _layer);
		handle.insertBefore(etr , h);
		
		//print element's attribute
		var atts = _element.attributes;
		for(var j=0 ; j < atts.length ; j++){
			var attr = atts[j];
			handle.insertBefore(_attributeToTr(attr , _layer) , h);
		}
	}

...
function ServiceDefinitionS2H(_domTree , _handle , _imagePrefix){
	...
	//remove all emements
	this.cleanUp = function(){
		var thead = handle.getElementsByTagName("thead")[0];
		var tbody = handle.getElementsByTagName("tbody")[0];
	...
	this.openElement = function(_element){
		...
		if(attributes.length > 0){
			var prefix = _HTML_TR + "-" + _element.getId() + "-" + attributes[0].id + "-a";
			var attr = document.getElementById(prefix);
			if(attr != null) justOpen = true;
		}
		...
		if(justOpen){
			...
			_printElementItem(lTr , _element);
			...
			for(var i=0;i<childrens.length;i++){
				if(!childrens[i].isLeaf){
					...
					handle.insertBefore(_elementToTr(childrens[i]) , _target);	

					//print the attribute scope
					//print fist tr
					handle.insertBefore(_createFirstTr(childrens[i] , "first") , _target);
					//print last tr
					handle.insertBefore(_createLastTr(childrens[i] , "last") , _target);

					//print the child elements scope
					handle.insertBefore(_createFirstTr(childrens[i] , "c-first") , _target);
					handle.insertBefore(_createFirstTr(childrens[i] , "c-last") , _target);

				}
			}
			//print last tr
			var lTarget = handle.insertBefore(_createLastTr(_element , "c-last") , _target);			
			*/
		}
	}
	...
	//Private methods: Add & Remove element methods
	//------------------------------------------------------------------------------------
	//------------------------------------------------------------------------------------
	_addElementAtFirst = function(_element , _eName){
		/*
	...
	_addElementAtLast = function(_element , _eName){

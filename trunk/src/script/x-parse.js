...
function MultiNSSchemaParse(_sXmlDoc , _nameSpace , _standNS){
	...
	this.findRootElement = function(){
		var r = null;
		var schemaNodes = getElementsByTagName(this.sXmlDoc, "schema", this.standNS);
		...
		if(schemaNodes.length >0){
			...
			for(var i=0;i<childs.length;i++){
				if(childs[i].nodeType != 3){
					if(childs[i].nodeName == (_getQName(this.standNS , "element"))){
						index = i;
						if(n > 1){ 
							return null;
						}else{
	...
	this.findElementByName = function(_elementName){	
		...
		if(ns == this.nameSpace){
			...
			var elements = getElementsByTagName(this.sXmlDoc, _SCHEMA_ELEMENT, this.standNS);
		...
		if(sNode != null){
			...
			for(var i=0;i<childs.length;i++){
				...
				if(child.nodeType != 8 && child.nodeType != 3 && 
					child.nodeName == _getQName(this.standNS ,"element")){
			...
			if(choice != null){
				...
				for(var i=0;i<childs.length;i++){
					var child = childs[i];
					if(child.nodeType != 3 && child.nodeName == _getQName(this.standNS , "element")){
						r.push(child);
					}
				}
			}
		...
		if(complexType != null){
			...
			if(simpleContent == null){
				...
				for(var i=0;i<attrs1.length;i++){
					if(attrs1[i].nodeType != 3 && attrs1[i].nodeName == _getQName(this.standNS , "attribute")){
						r.push(attrs1[i]);
					}
				}
			}else{
				...
				for(var i=0;i<attrs2.length;i++){
					if(attrs2[i].nodeType != 3 && attrs2[i].nodeName == _getQName(this.standNS , "attribute")){
						r.push(attrs2[i]);
					}
				}
			}
	...
	this.findSimpleType = function(_sName){
		...
		var root = getElementsByTagName(this.sXmlDoc, "schema", this.standNS);
		
		//find <xsd:simpleType>
		var name = _getElementName(_sName);			
		var childs = root[0].childNodes;
		for(var i=0;i<childs.length;i++){
			if(childs[i].nodeType != 3 && childs[i].nodeName == _getQName(this.standNS , "simpleType")){
	...
	this.findElementChildrenContentRule = function(_elementName , _childName){
		...
		//find rule
		var rule = null;
		var childs = getElementsByTagName(sNode, "schema", this.standNS);
		*/
		...
		for(var i=0;i<childs.length;i++){
			...
			if(child.nodeType != 3){	
				...
				if(ref != null){
					...
					if(_childName == _getQName(nameSpace , name)){
						var ns = _getNS(type);
						if(ns == this.standNS){
							//basic types
	...
	_getFirstNodeOfElement = function(_element , _nodeName){
		...
		for(var i=0;i<nodes.length;i++){
			if(nodes[i].nodeType != 3 && nodes[i].nodeName == _getQName(_this.standNS , _nodeName)){
				return nodes[i];
			}
		}
		return r;
	...
	_getQName = function(_prefix , _refName){
		return _prefix + ":" + _refName;
	}
}